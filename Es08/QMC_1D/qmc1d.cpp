...
double external_potential(double val)
{
    ...
    return (val*val*val*val)-((5.0/2.0)*val*val);
}
...
double variationalWaveFunction_second(double v)
{
    ...
    return (-1./pow(sigma,2.)*variationalWaveFunction(v)+pow(v-mu,2.)/pow(sigma,4.)*exp(-0.5*pow((v-mu)/sigma,2.))+pow(v+mu,2.)/pow(sigma,4.)*exp(-0.5*pow((v+mu)/sigma,2.)));
}
...
void readInput()
{
	...
	ifstream input_file("input.pimc");
