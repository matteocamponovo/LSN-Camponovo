...
//costruttore per crossover
Chrom::Chrom( Chrom& parent1,  Chrom& parent2, int cross, Random *Srnd){
...
void Chrom::Load_Element0(int var){
        vect.erase(vect.begin(), vect.end());
        vect.push_back(var);
}

void Chrom::Load_Element(int var){
        vect.push_back(var);
}
...
void Chrom::Save_conf(const Maps Cities, int my_rank, int rect){
	ofstream Conf_cit;
    Conf_cit.open("Maps_"+to_string(my_rank)+".dat");
...
void Chrom::Save_conf_finale(const Maps Cities){
    ofstream Conf_cit("Maps_finale");
    for(int i=0;i<m_N;i++)    Conf_cit<<setprecision(9)<<i<<"\t"<<Cities.GetPos(vect.at(i)).x<<"\t"<<Cities.GetPos(vect.at(i)).y<<endl;
        Conf_cit<<setprecision(9)<<m_N<<"\t"<<Cities.GetPos(vect.at(0)).x<<"\t"<<Cities.GetPos(vect.at(0)).y<<endl;
        Conf_cit.close();
}
