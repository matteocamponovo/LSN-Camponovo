...
int main(){
  ...
  int nconf = 1;
  for(int istep=1; istep <= nstep; ++istep){  //nstep è il numero di step totali della simulazione
...
void Measure(){ //Properties measurement
  ...
  t = 0.0;
   ...
   for(int i=0; i<nbins; i++){ //reset the hystogram of g(r)
      walker[i]=0;
   }
  ...
  for (int i=0; i<npart-1; ++i){
    for (int j=i+1; j<npart; ++j){
     ...
     dr = sqrt(dr);
      ...
      if (dr<box/2.0){     //per il calcolo della g(r)
           bin=(int)(dr/bin_size);
...
void Accumulate(){
    ...
    ave_temp+=stima_temp;
    ave_etot+=stima_etot;
    for(int i=0; i<nbins; ++i) blk_av[i] = blk_av[i] + walker[i];
...
void Blocco(double b){
    ...
    for (int i=0; i<nbins; i++){
        stima_gdir=(double)(3.0/(rho*npart*4.0*M_PI*((bin_size*(i+2))*(bin_size*(i+2))*(bin_size*(i+2))-(bin_size*(i+2-1))*(bin_size*(i+2-1))*(bin_size*(i+2-1)))))*blk_av[i]/(blk_norm);
        glob_av[i]+=stima_gdir;
        glob_av2[i]+=stima_gdir*stima_gdir;
    }

    if (b==nblocks) {   //questa è come se fosse la media a blocchi della g(r)
            out.open("ave_gdir_gas.out", ios::app);
            for(int i=0; i<nbins; i++){
