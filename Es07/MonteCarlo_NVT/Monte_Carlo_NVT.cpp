...
using namespace std;
                        //manca da calcolare la g(r) e la pressione nella 4!!!!
int main()
{ 
  ...
  int nconf = 1;
    ...
    cout << "ORA INZIA LA SIMULAZIONE" <<endl<<endl;
    //******************************************************************************
...
{
 // ofstream out;
 // out.open("U_P_instant_gas.txt",ios::app);
  int bin;
  double v = 0.0, w = 0.0;
  double vij, wij;
  double dx, dy, dz, dr;
...
// contribution to energy and virial
  ...
  //out << walker[iv]/(double)npart + vtail << "    " << rho * temp + (walker[iw] + (double)npart * ptail) / vol << endl;
 // out.close();
}
...
void Averages(int iblk) //Print results for current block
{
   ...
   const int wd=15;
    ...
    Gofr.open("output.gofr_gas.0",ios::app);
    Gave.open("output.gave_gas.0",ios::app);
    
    stima_pot = blk_av[iv]/blk_norm/(double)npart + vtail; //Potential energy
    glob_av[iv] += eKb*1.380649*pow(10,-23)*stima_pot;
    glob_av2[iv] += stima_pot*stima_pot*eKb*1.380649*pow(10,-23)*eKb*1.380649*pow(10,-23);
    err_pot=Error(glob_av[iv],glob_av2[iv],iblk);
    
    stima_pres = rho * temp + (blk_av[iw]/blk_norm + ptail * (double)npart) / vol; //Pressure
    glob_av[iw] += (eKb/(sigma*sigma*sigma))*1.380649*pow(10,-23)*stima_pres;
    glob_av2[iw] += pow((eKb/(sigma*sigma*sigma))*1.380649*pow(10,-23),2)*stima_pres*stima_pres;
...
//g(r)
    for (int i=igofr; i<igofr+nbins; ++i){
        stima_gdir=(double)(3.0/
                            (rho*npart*4.0*M_PI* ((bin_size*i)*(bin_size*i)*(bin_size*i)-(bin_size*(i-1))*(bin_size*(i-1))*(bin_size*(i-1)))))*blk_av[i]/blk_norm;
        glob_av[i]+=stima_gdir;
        glob_av2[i]+=stima_gdir*stima_gdir;
        Gofr << (bin_size*(i)+bin_size/2.0)*sigma << "    " << stima_gdir  << endl;
    }
    Gofr << endl;

    if (iblk==nblk) {    //questa è come se fosse la media a blocchi della g(r)
        int b=iblk;
        for(int i=igofr; i<igofr+nbins; ++i){
            err_gdir = sqrt((glob_av2[i]/(double)b - glob_av[i]*glob_av[i]/((double)b*b)) / double(b-1));
            Gave << (bin_size*(i-2)+bin_size/2.0)*sigma << "    " << glob_av[i]/(double)b << "    " << err_gdir << endl;
        }
    }
